...
procedure TUser2ReviewForm.FormKeyUp(Sender: TObject; var Key: Word; var KeyChar: Char; Shift: TShiftState);
begin
  if Key = 137 then
    self.Free;
end;
...
procedure TUser2ReviewForm.TMSFMXRatingTimeValueClick(Sender: TObject; AValue: Single);
begin
  self.v_Item1 := AValue;
  self.calculate;
end;

procedure TUser2ReviewForm.TMSFMXRatingQualityValueClick(Sender: TObject; AValue: Single);
begin
  self.v_Item2 := AValue;
  self.calculate;
end;

procedure TUser2ReviewForm.TabControl1Change(Sender: TObject);
begin
  if TabControl1.ActiveTab = TabItemRating then
  begin
    TabItemRating.ImageIndex := 27;
    TabItemFeedback.ImageIndex := 28;
  end
  else if TabControl1.ActiveTab = TabItemFeedback then
  begin
    TabItemRating.ImageIndex := 25;
    TabItemFeedback.ImageIndex := 26;
  end
end;

procedure TUser2ReviewForm.TMSFMXRatingPriceValueClick(Sender: TObject; AValue: Single);
begin
  self.v_Item3 := AValue;
  self.calculate;
end;
...
begin
  ...
  LabelSum.Text := self.v_Item1.ToString + '+' + self.v_Item2.ToString + '+' + self.v_Item3.ToString + ' = ' +
    self.v_total.ToString;
end;
